...
(()=>{
 ...
 fetch('quests-data.json?v=2').then(r=>{if(!r.ok)throw Error('Quest links unavailable');return r.json();}).then(pack=>{
  function expand(q,records){const rewards=[...(q.rewards||[])];if(q.rewardMod)for(const r of records){const raw=r.raw||r.r;if(Number(raw['LO #'])===q.rewardMod.lo)rewards.push({catalog:r.id,kind:(q.rewardMod.boss||[]).includes(r.id)?'boss':q.rewardMod.kind,note:q.rewardMod.note});}return{...q,rewards};}
