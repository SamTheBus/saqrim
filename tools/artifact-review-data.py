...
SCOPE='Author-reference Artificer 1.0.11; recorded PS5 menu v1.00 does not identify its upstream build. Current 23 September load-order priority is applied, but this is still a documentation-based expectation, not an inspected winning plugin record or an in-game measurement.'
...
def build(original):
 ...
 for n,(name,kind,effect,where,region,route,conflict) in enumerate(ADDITIONS,618):
  ...
  r={'Catalog ID':ident,'Target':name,'Type':kind,'Browse category':category,'LO #':'58','Installed mod':'Artificer - An Artifact Overhaul','Effect / interest':effect or 'Final enchantment and stats unresolved: ArteFakes #57 also edits this named item.','Where / unlock':where,'When to look':'Follow the named quest or dungeon route; do not infer a level recommendation.','Acquisition':'Existing unique / quest or dungeon loot','Region / route':region,'Evidence status':'Expected from documentation; PS5 records uninspected','Qualifications':SCOPE,'Source scope':'Mod-author effect documentation; base-game route reference separately identified.','Source URL':ART+'\n'+route,'Found?':'Not checked','My choice':'Undecided','Checked on':'2026-09-18'}
  rows.append(r);by[ident]=r
 def audit(ident,origin='Changed vanilla / DLC unique',status='Expected from documentation'):
  r=by[ident]
  obj={'origin':origin,'status':status,'reviewed':'2026-09-18','basis':'Documentation + current 211-entry order derived from the recorded baseline; no plugin-record or gameplay verification','expected':'Artificer #58 is the documented effect source. No later item-specific edit has been established by the reviewed descriptions for this entry. This is not proof that the rest of the load order contains no override.','chain':'#58 Artificer; #59 Artificer–USSEP patch is present, but its per-item forwarding has not been inspected.','appearance':'Model and texture resolution is separate. #55–56 Xavbio loads before #57 ArteFakes and #58 Artificer; exact asset paths and the PS5 port payload remain uninspected.','acquisition':'The documented acquisition route is listed separately from effects. Later quest, NPC, inventory and world-reference edits are not certified by this item review.','verification':SCOPE,'sources':[ART,XEDIT],'new':int(ident[1:])>=618}
 ...
 for ident,name in OVERLAPS.items():
  ...
  r=by[ident]
  if ident=='W180':
   a.update(status='Documented overlap',expected='Steel Battleaxe of Fiery Souls – Truly Unique #68 is the latest documented item-specific editor after ArteFakes #57 and Artificer #58. Its Emberwisp / Flameclaim version is therefore the expected candidate, but the exact PS5 winning records remain uninspected.',chain='#57 ArteFakes → #58 Artificer → #59 Artificer–USSEP scope uninspected → #68 Fiery Souls – Truly Unique.',appearance='The dedicated #68 overhaul is also later than ArteFakes. Do not assume ArteFakes supplies the final model or that Artificer\'s Soulbrand fields are merged into #68.',verification='Recorded PS5 menu versions establish order, not record contents. The dedicated axe is the latest documented same-item editor, so its mechanics are the expectation rather than a certified plugin scan.',sources=[ART,FAKES,AXE,XEDIT])
   a['earlierEffect']='Artificer #58 separately renames and reworks this weapon as Soulbrand; those earlier fields are not assumed to survive #68.'
  else:
   a.update(status='Documented overlap',expected='Artificer #58 loads after ArteFakes #57. If both PS5 ports override the same base-item record, Artificer is the later documented record candidate, so its mechanics are the expected version. ArteFakes\' model assignment is not guaranteed to survive without a compatibility patch.',chain='#57 ArteFakes → #58 Artificer → #59 Artificer–USSEP scope uninspected. No separately listed Artificer–ArteFakes reconciliation patch is present in the current load order.',appearance='A public Artificer–ArteFakes compatibility patch exists specifically to carry ArteFakes models into Artificer. It is not installed here, so the site does not promise ArteFakes visuals on these overlapping items.',verification='This is load-order precedence from documented overlap, not an inspected PS5 plugin record. The exact port payload and any asset-only wins remain unverified.',sources=[ART,FAKES,PATCH,XEDIT])
   a['earlierEffect']='ArteFakes #57 is earlier and model-focused. Without the compatibility patch, its appearance is not assumed to survive the later Artificer item record.'
   r['Effect / interest']='Expected Artificer reference: '+r['Effect / interest']
 for ident in RINGS:
  a=audits[ident];a.update(status='Possible later edit',expected='Artificer #58 documents this ring, but Wear Multiple Rings #96 is a later equipment editor. The exact ring records it includes and whether it forwards the enchantment are unknown; no winner is certified.',chain='#58 Artificer → #59 Artificer–USSEP scope uninspected → #96 Wear Multiple Rings (possible ring-record overlap, NOT a proven same-record edit).',sources=[ART,VIDEO+'&t=116s',XEDIT])
  a['earlierEffect']=by[ident]['Effect / interest'];by[ident]['Effect / interest']='Final ring effect unresolved · #96 Wear Multiple Rings may affect this item; the Artificer reference is retained below, not certified as the winning effect.'
 for i in range(39,49):
  ident=f'W{i:03}';a=audit(ident,'Replacement thane reward','Documented overlap')
  a.update(expected='Unique Thane Weapons #69 is the later documented thane-reward overhaul. Expect its reward assignment only if its quest/award records win. Do not stack its enchantment with Artificer\'s or assume an already received reward is retroactively replaced.',chain='#58 Artificer → #69 Unique Thane Weapons; reward-selection and item records need separate verification.',sources=[ART,THANE,XEDIT],earlierEffect=by[ident]['Effect / interest'])
  by[ident]['Effect / interest']='Expected #69 thane-reward candidate; final quest assignment and enchantment remain unverified. See the load-order review.'
 for ident in [f'W{i:03}' for i in range(358,365)]:
  a=audit(ident,'Creation relic','Identity / acquisition check');a.update(expected='Knight of the North #145 documents the relic hunt and access conditions. It is the acquisition reference here, not proof of a final artifact-stat winner.',chain='Divine Crusader Creation → #145 Knight of the North acquisition overhaul; intervening equipment and later references remain uninspected.',sources=['https://www.nexusmods.com/skyrimspecialedition/articles/3005',XEDIT],verification='Creation Club relic, not a vanilla Skyrim/DLC unique. Recorded PS5 v1.00 does not establish an exact PC branch.')
 for ident in ['W050','W052','W177','W178','W179','W181','W187']:
  ...
  if ident in ['W050','W052']:
   a['expected']='Both Artificer #58 and ArteFakes #57 name this restored or added item. Shared identity has not been established: they may target a shared restored record or introduce separate copies. The name alone does not prove the two ports use the same record; if they do, #58 Artificer is later.';a['sources'] += [FAKES]
  if ident=='W177':a['expected']='Volkihar Relic Sword #71 is documented as its own relic story. It is NOT established as a replacement for Harkon\'s Sword (W547).'
  if ident=='W187':a['expected']='Halidil\'s shield is an alternative acquisition lead. An NPC carrying an Aetherial Shield does not prove Dwarven Power Armor #76 overwrites the original shield\'s ARMO record or Artificer effect (W311).'
 # Additional current-order conflicts found after the first artifact pass.
 a=audits['W627']
 a.update(status='Documented overlap',expected='Artificer #58 is later than Praedy\'s Staves #50–54. Artificer\'s Staff of Magnus mechanics are the expected record outcome, but Praedy\'s model is not guaranteed because published Praedy/Artificer compatibility patches exist and none is present in this load order.',chain='#50 Praedy\'s Staves → #51 Apocalypse patch → #52 Mysticism patch → #53 Odin patch → #54 USSEP patch → #58 Artificer. No Artificer–Praedy patch appears in the current order.',appearance='Praedy documents that staves edited by other mods can need compatibility patches, and its patch hub includes Artificer. Treat the Staff of Magnus appearance as unresolved while retaining Artificer\'s documented mechanics.',sources=[ART,PRAEDY,PRAEDY_PATCH,XEDIT],earlierEffect='Praedy\'s Staff of Magnus model is installed earlier; without the missing Artificer compatibility patch it is not promised to survive the later item edit.')
 ...
 for target in ['Windshear',"Firiniel's End"]:
  ...
  a.update(expected='JaySerpa\'s Quest Expansion Bundle #131 loads after Artificer #58. A published compatibility patch states that Destroy the Dark Brotherhood – Quest Expansion can prevent this Artificer artifact from being obtainable. The specific Artificer compatibility patch is not present in the current 211-mod order.',chain='#58 Artificer → #131 JaySerpa Quest Expansion Bundle. #132 Wintersun and #133 USSEP are bundle patches, not the separate Artificer compatibility patch.',acquisition='Acquisition is the conflict here: the later quest expansion can remove the NPC/item handoff used by Artificer. Do not promise this pickup until the PS5 bundle is checked or a matching compatibility patch is added.',verification='The incompatibility is documented for the upstream mods. The exact PS5 bundle/port contents have not been inspected, so the site labels the pickup at risk rather than certifying it absent.',sources=[ART,JAY_ART,XEDIT],earlierEffect=r['Effect / interest'])
