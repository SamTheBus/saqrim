"""Validate artifact/site precedence against Sam's current 21 September load order."""
...
bypos=dict(lo)
expected={32:'Triumvirate - Mage Archetypes',33:'Mysticism - A Magic Overhaul',34:'Mysticism - Vokrii Patch',35:'Mysticism - Ordinator Patch',36:'Odin - Skyrim Magic Overhaul',37:'Odin - Vokrii Compatibility Patch',38:'Odin - Ordinator Compatibility Patch',39:'Apocalypse - Magic of Skyrim',40:'Apocalypse - Ordinator Compatibility Patch',41:'Apocalypse - Vokrii Compatibility Patch',42:'Ordinator - 50 Percent More Perk Points',43:"Kittytail's Spells AIO [PS5]",55:"Praedy's Staves - USSEP Patch",56:'ArteFakes - Unique Artifacts Replacer',57:'Artificer - An Artifact Overhaul',58:'Artificer - USSEP Patch',59:'Guns of Skyrim (formerly Lore Friendly Rifles)',67:'Steel Battleaxe of Fiery Souls - Truly Unique (PS5)',68:'Unique Thane Weapons (PS5)',99:'Dwarven Automatons Pack - Mihail',100:'Bone Colossus - Necromantic Grimoire Integration',214:'Enhanced Blood Textures Lite',215:'Vokrii - Minimalistic Perks of Skyrim',216:'Ordinator - Perks of Skyrim',217:'Ordinator - Fix - PS5 (textures)',218:'Ordinator - Bone Collector (Windhelm altar fix)',219:'Vokriinator [EN]'}
check('219 unique current load-order positions',len(lo)==219 and [n for n,_ in lo]==list(range(1,220)))
...
check('Knight relic labels use #153',all(by[f'W{i:03}']['LO #']=='153' for i in range(358,365)))
...
for ident in ['W307','W310','W367','W368','W542']:
 check(ident+' ring warning renumbered','#104 Wear Multiple Rings' in audit[ident]['chain'] and '#57 Artificer' in audit[ident]['chain'])
...
check('Destroy Dark Brotherhood quest points to current #139',q30['lo']==139)
...
with sync_playwright() as p:
 ...
 page.evaluate('localStorage.clear();localStorage.setItem("saqrim-load-order-220-2026-09-18-v2","[99,100,101,220]")');page.reload();page.wait_for_selector('#mod-100');check('previous checklist drops removed entry and shifts later positions',page.is_checked('#mod-099 input[type=checkbox]') and page.is_checked('#mod-100 input[type=checkbox]') and page.is_checked('#mod-219 input[type=checkbox]') and page.evaluate('JSON.parse(localStorage.getItem("saqrim-load-order-219-2026-09-21-v3")).length===3'))
 ...
 page.goto(URL+'quests.html#Q030');page.wait_for_function('window.SaqrimQuests');page.wait_for_selector('#Q030[open]');check('quest links current load order #139',page.locator('#Q030 a[href="load-order.html#mod-139"]').count()==1)
